...
namespace SageOfAstra.Configs
{
    public class CoreConfig
    {
        ...
        public static async Task<CoreConfig> CreateNewAsync()
        {
            ...
            Console.WriteLine("Sage of Astra's token: ");
            result.Token = Console.ReadLine();
            Console.WriteLine("Sage of Astra's command prefix: ");
            result.Prefix = Console.ReadLine();
            Console.WriteLine("Sage of Astra's master ID (will bypass all permission checks. Hymonsi's ID: 97434679916888064): ");
            result.MasterId = ulong.Parse(Console.ReadLine());
            Console.WriteLine("Mute role ID: ");
            result.MuteRoleId = ulong.Parse(Console.ReadLine());
            Console.WriteLine("Mention (@Sage of Astra) Sage of Astra as a substitute for a command prefix? (y/n): ");
